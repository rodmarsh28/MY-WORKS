Public Class frmMaterialReceiving
    ...
    Sub addtoDGW()
        ...
        dgw.Item(3, rows).Value = txtItemCategories.Text
        dgw.Item(4, rows).Value = txtUnit.Text
        
        dgw.Item(7, rows).Value = txtQuantity.Text
        If chkboxLessVat.Checked = True Then
            dgw.Item(5, rows).Value = txtUnitPrice.Text * 1.12
            dgw.Item(6, rows).Value = txtUnitPrice.Text * txtQuantity.Text
            dgw.Item(8, rows).Value = txtUnitPrice.Text * txtQuantity.Text
            totalAmount = totalAmount + txtUnitPrice.Text * txtQuantity.Text
        Else
            dgw.Item(5, rows).Value = txtUnitPrice.Text * 1
    ...
    Sub dgwItemProcess()
        ...
        While col < row1
            ...
            itemCategories = dgw.Item(3, col).Value
            UNIT = dgw.Item(4, col).Value
            If chkboxLessVat.Checked = True Then
                unitPrice = dgw.Item(6, col).Value
            Else
                unitPrice = dgw.Item(5, col).Value
            End If
            originalPrice = dgw.Item(5, col).Value
            LessVatAmount = dgw.Item(6, col).Value
            quantity = dgw.Item(7, col).Value
            amount = dgw.Item(8, col).Value
            saveItem()
        ...
        Try
            ...
            With OleDBC
                ...
                .CommandText = "SELECT dbo.tblMRSDESC.MRSNO,dbo.tblMRSDESC.[DATE],dbo.tblMRSDESC.POREFNO,dbo.tblMRSDESC.DR,dbo.tblMRSDESC.SUPPLIERNAME," & _
                    ...
                    "dbo.tblMRSITEM.SERIAL,dbo.tblMRSITEM.ITEMCATEGORIES,dbo.tblMRSITEM.UNIT,dbo.tblMRSITEM.QUANTITY FROM dbo.tblMRSITEM INNER JOIN dbo.tblMRSDESC ON dbo.tblMRSDESC.MRSNO = dbo.tblMRSITEM.MRSNO " & _
            ...
            If OleDBDR.HasRows Then
                While OleDBDR.Read
                    dt.Rows.Add(OleDBDR.Item(0), Format(OleDBDR.Item(1), "MM/dd/yyyy"), OleDBDR.Item(2), OleDBDR.Item(3), OleDBDR.Item(4), OleDBDR.Item(5), OleDBDR.Item(6),
                                 OleDBDR.Item(7), OleDBDR.Item(8), OleDBDR.Item(9), OleDBDR.Item(10), OleDBDR.Item(11), OleDBDR.Item(12), OleDBDR.Item(13))
                    c = c + 1
                End While
            End If
    ...
    Private Sub Button4_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button4.Click
        ...
        frmFindMaterials.dgw.Columns(5).HeaderCell.Value = "UNIT PRICE"
        frmFindMaterials.dgw.Columns(5).DefaultCellStyle.Format = "c"
    ...
    Private Sub DELELTEITEMToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles DELELTEITEMToolStripMenuItem.Click
        ...
        For Each row As DataGridViewRow In dgw.SelectedRows
            totalAmount = totalAmount - dgw.CurrentRow.Cells(8).Value
            haveDatabase()
            dgw.Rows.Remove(row)
            rows = rows - 1
            lblTotal.Text = "TOTAL FOR THIS PURCHASE ORDER :  ₱ " & totalAmount.ToString("n2")
