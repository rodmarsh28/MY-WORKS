Public Class frmPayroll
    ...
    Sub selectLastNetpay()
        ...
        Try
            ...
            With OleDBC
                .Connection = conn
                .CommandText = "select * from tblPayrollInfo where dateTo = #" & dtr1 & "# or dateTo = #" & dtr2 & "# and employeeID  = '" & empid & "'"
            End With
            OleDBDR = OleDBC.ExecuteReader
            If OleDBDR.HasRows Then
                If OleDBDR.Read Then
                    lastnetpay = OleDBDR.Item(18)
                End If
            End If
    ...
    Sub deductions()
        ...
        Else
            ...
            basicpay = (txtDR.Text / 2) - (txtDR.Text / 365 * 12 * regularWorkedDays)
            latecash = (txtDR.Text / 365 * 12 / 8 / 60) * late
            regularholiday = (txtDR.Text / 365 * 12) * regularHolidays
            nonworkingholiday = (txtDR.Text / 365 * 12) * 0.3 * nonWorkingHolidays
            leavepaycash = (txtDR.Text / 365 * 12) * leavePay
        End If
    ...
    End Sub
    Sub selectExist()
        Dim dtr1 As Date = Format(dtrFrom.Value, "Short Date")
        Dim dtr2 As Date = Format(dtrTo.Value, "Short Date")
        Try
            ...
            With OleDBC
                .Connection = conn
                .CommandText = "select * from tblPayrollInfo where dateFrom = #" & dtr1 & "# and employeeID  = '" & txtemID.Text & _
                    "' or dateTo = #" & dtr2 & "# and employeeID  = '" & txtemID.Text & "'"
            End With
            OleDBDR = OleDBC.ExecuteReader
            If OleDBDR.HasRows Then
        ...
        Else
            If MsgBox("Save Payroll?", MsgBoxStyle.Question + MsgBoxStyle.YesNo, "WARNING") = MsgBoxResult.Yes Then
                ...
                Try
                    ...
                    With OleDBC
                        .Connection = conn
                        .CommandText = " INSERT INTO tblPayrollInfo(employeeID,dateFrom,dateTo,regularWorkedDays,regularHolidays,nonWorkingHolidays,leavePay," & _
                            "overtime,grossPay,lateUndertime,withHoldingTax,SSS,pagibig,philhealth,SSSLoan,pagibigLoans,cashAdvance,totalDeductions,netPay,restday) VALUES ('" & txtemID.Text & _
                                "','" & dtrFrom.Text & _
                                "','" & dtrTo.Text & _
                                "','" & regularwork & _
                ...
                Try
                    ...
                    With OleDBC
                        .Connection = conn
                        .CommandText = "insert into tblPayrollCash(payrollID,regularworkedCash,regularHolidaysCash,nonWorkingHolidaysCash,leavePayCash,overtimeCash,lateUndertimeCash) VALUES('" & id & _
                            "','" & Format(basicpay, "0.00") & _
                            "','" & Format(regularholiday, "0.00") & _
                            "','" & Format(nonworkingholiday, "0.00") & _
                            "','" & Format(leavepaycash, "0.00") & _
                            "','" & Format(overtimecash, "0.00") & _
                            "','" & Format(latecash, "0.00") & "')"
                        .ExecuteNonQuery()
                    End With
                    MsgBox("PAYROLL GENERATED !", MsgBoxStyle.OkOnly, txtName.Text)
                    clear()
                Catch ex As Exception
                    MsgBox(ex.Message)
                Finally
    ...
    Sub selectlastid()
        ...
        Try
            ...
            With OleDBC
                .Connection = conn
                .CommandText = "select * from tblPayrollInfo order by payrollID desc"
            End With
            OleDBDR = OleDBC.ExecuteReader
            If OleDBDR.HasRows Then
                If OleDBDR.Read Then
